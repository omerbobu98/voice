...
STORY_GENERATOR_PROMPT = """You are an expert sales storyteller for home improvement sales.
Your job is to create compelling, emotionally resonant stories that salespeople can use to overcome objections and close deals.

## The 6 Elements of a Perfect Sales Story:
1. **RELATABLE CHARACTER** - A real-sounding name, specific location, similar situation to customer
2. **THE HESITATION** - They had the EXACT same objection the customer has now
3. **THE DECISION MOMENT** - What finally made them decide
4. **THE COST OF WAITING** - What they lost or almost lost by hesitating
5. **THE TRANSFORMATION** - Specific, measurable results (with numbers and timeframes)
6. **THE EMOTIONAL PAYOFF** - How they FEEL now (peace of mind, pride, relief)
...
## Your Task:
Create a story based on these inputs:
- Target Emotion: {target_emotion}
- Main Message: {target_message}
...
{{
    "title": "שם קצר לסיפור (3-5 מילים)",
    "story_content": "הסיפור המלא בעברית (60-90 שניות כשנאמר בקול)",
    "story_structure": {{
        "character": "שם ותיאור הדמות",
        "hesitation": "ההיסוס שלהם",
        "decision_moment": "מה גרם להם להחליט",
        "cost_of_waiting": "מה הפסידו/כמעט הפסידו",
        "transformation": "התוצאות הספציפיות",
        "emotional_payoff": "איך הם מרגישים עכשיו"
    }},
    "key_quote": "ציטוט מפתח מהסיפור שאפשר להשתמש בו",
    "when_to_use": "מתי להשתמש בסיפור הזה",
    "tags": ["tag1", "tag2", "tag3"]
}}

Make the story:
- Sound REAL and SPECIFIC (use realistic names, areas, numbers)
...
def generate_story():
    """Generate a new story based on emotion/message inputs"""
    ...
    data = request.get_json()
    target_emotion = data.get('target_emotion', 'trust')
    ...
    try:
        ...
        response = openai_client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": "You are an expert sales storyteller. Respond ONLY with valid JSON in Hebrew."},
                {"role": "user", "content": prompt}
            ],
            temperature=0.8,
            max_tokens=2000
        ...
        print(f"[generate_story] JSON parse error: {e}")
...
def save_story():
    """Save a generated story to the story bank"""
    ...
    try:
        story_record = {
            'user_id': user_id,
            'title': data.get('title', 'סיפור חדש'),
        ...
        result = client.table('story_bank').insert(story_record).execute()
...
def update_story(story_id):
    """Update a story in the story bank"""
    ...
    try:
        ...
        if 'story_content' in data:
            update_data['story_content'] = data['story_content']
...
def increment_story_usage(story_id):
    """Increment the usage count for a story"""
    ...
    try:
        # Get current count
        result = client.table('story_bank').select('usage_count').eq('id', story_id).eq('user_id', user_id).execute()
        if result.data:
            current_count = result.data[0].get('usage_count', 0)
            client.table('story_bank').update({'usage_count': current_count + 1}).eq('id', story_id).execute()
